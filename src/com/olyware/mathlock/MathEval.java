...
public class MathEval extends Object {
	...
	/**
	 * Set a named constant (constant names are not case-sensitive). Constants are like variables but are not cleared by clear(). Variables
	 * of the same name have precedence over constants.
	 */
	...
	/**
	 * Set a named constant (constants names are not case-sensitive). Constants are like variables but are not cleared by clear(). Variables
	 * of the same name have precedence over constants.
	 */
	...
	/**
	 * Set a pure function handler for the specific named function, replacing any existing handler for the given name; if the handler is
	 * null the function handler is removed.
	 * <p>
	 * Pure functions have results which depend purely on their arguments; given constant arguments they will have a constant result. Impure
	 * functions are rare.
	 */
	...
	/**
	 * Set a function handler for the specific named function optionally tagging the function as impure, replacing any existing handler for
	...
	/**
	 * Get whether a variable which is used in an expression is required to be explicitly set. If not explicitly set, the value 0.0 is
	 * assumed.
	 */
	...
	/**
	 * Set whether a variable which is used in an expression is required to be explicitly set. If not explicitly set, the value 0.0 is
	 * assumed.
	 */
	...
	/**
	 * Return whether the previous expression evaluated was constant (i.e. contained no variables). This is useful when optimizing to store
	 * the result instead of repeatedly evaluating a constant expression like "2+2".
	 */
	...
	private double _evaluate(int beg, int end, double lft, Operator pnd, Operator cur) throws NumberFormatException, ArithmeticException {
		...
		for (ofs = beg; (ofs = skipWhitespace(expression, ofs, end)) <= end; ofs++) {
			...
			if (opPrecedence(cur, LEFT_SIDE) < opPrecedence(nxt, RIGHT_SIDE)) {                              // correct even for last (non-operator) character, since
				// non-operators have the artificial "precedence" zero
	 ...
	 * Operator Structure.
	 * <p>
	 * This class is immutable and threadsafe, but note that whether it can be used in multiple MathEval instances (as opposed to for
	 * multiple operators in one instance) depends on the threadsafety of the handler it contains.
	 */
	static public final class Operator extends Object {
		 ...
		 * <p>
		 * Flagging an operator as unary binds the precedence for the specified side such that it always has maximum precedence when
		 * considered from the opposite side.
		 */
	...
	static class DefaultImpl extends Object implements OperatorHandler, FunctionHandler {
		...
		public double evaluateFunction(String fncnam, ArgParser fncargs) throws ArithmeticException {
			...
			case 'm': {
				...
				break;
			case 'n': {
				// if(fncnam.equalsIgnoreCase("nextUp" )) { return Math.nextUp (fncargs.next()); } //needs API min 9
			}
				break;
			case 'r': {
			...
			throw new UnsupportedOperationException("MathEval internal function setup is incorrect - internal function \"" + fncnam
					+ "\" not handled");
		}

		static final DefaultImpl INSTANCE = new DefaultImpl();

		static private final Operator OPR_EQU = new Operator('=', 99, 99, RIGHT_SIDE, true, DefaultImpl.INSTANCE); // simple assignment,
																													// used as the final
																													// operation, must be
																													// maximum precedence
	...
	static private final Operator OPERAND = new Operator('\0', 0, 0, NO_SIDE, false, null);          // special "non-operator" representing an
	// operand character
