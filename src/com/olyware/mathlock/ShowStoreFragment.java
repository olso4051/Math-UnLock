...
public class ShowStoreFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
		...
		initMoney();
		costSmall = (TextView) view.findViewById(R.id.cost_small);
		costLarge = (TextView) view.findViewById(R.id.cost_large);
		costAll = (TextView) view.findViewById(R.id.cost_all);
		...
		buttonCoins1 = (Button) view.findViewById(R.id.extra_coins1);
		buttonCoins1.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				purchaseCoins(getActivity(), SKU[0], Cost[0] + 1, mPurchaseFinishedListener, PURCHASE_KEY);
			}
		});
		buttonCoins2 = (Button) view.findViewById(R.id.extra_coins2);
		buttonCoins2.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				purchaseCoins(getActivity(), SKU[1], Cost[1] + 1, mPurchaseFinishedListener, PURCHASE_KEY);
			}
		});
		buttonCoins3 = (Button) view.findViewById(R.id.extra_coins3);
		buttonCoins3.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				purchaseCoins(getActivity(), SKU[2], Cost[2] + 1, mPurchaseFinishedListener, PURCHASE_KEY);
			}
		});

		buy = new TextView[unlockCost.length];
		cost = new TextView[unlockCost.length];
		buy_layout = new RelativeLayout[unlockCost.length];
		layout_ids = new int[unlockCost.length];
		layout_ids[0] = R.id.pnlBuyAll;
		layout_ids[1] = R.id.pnlBuyMath;
		layout_ids[2] = R.id.pnlBuyVocab;
		layout_ids[3] = R.id.pnlBuyLanguage;
		layout_ids[4] = R.id.pnlBuyEngineer;
		layout_ids[5] = R.id.pnlBuyHiqTravia;
		layout_ids[6] = R.id.pnlBuyHiqTraviaExpansion;

		packsTitle = ((TextView) view.findViewById(R.id.packs));
		packsTitle.setPaintFlags(packsTitle.getPaintFlags() | Paint.UNDERLINE_TEXT_FLAG);

		for (int a = 0; a < buy.length; a++) {
			final int loc = a;
			int idButton = getResources().getIdentifier(unlockPackageKeys[a], "id", getActivity().getPackageName());
			int idText = getResources().getIdentifier(unlockPackageKeys[a].substring(7) + "_cost", "id", getActivity().getPackageName());
			cost[a] = (TextView) view.findViewById(idText);
			buy[a] = (TextView) view.findViewById(idButton);
			buy_layout[a] = (RelativeLayout) view.findViewById(layout_ids[a]);
			buy_layout[a].setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					if (cost[loc].getText().equals("ON") && (loc != 0)) {

						SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(getActivity());
						preferences.edit().putBoolean(PackageKeys[loc - 1], !preferences.getBoolean(PackageKeys[loc - 1], false)).commit();
						cost[loc].setText("OFF");
						// Toast.makeText(getActivity(), "" + PackageKeys[loc - 1], 1).show();

						Drawable drawable_unselected = getResources().getDrawable(R.drawable.oval_unselected);
						cost[loc].setCompoundDrawablesWithIntrinsicBounds(null, drawable_unselected, null, null);

					} else if (cost[loc].getText().equals("OFF") && loc != 0) {
						Drawable drawable_selected = getResources().getDrawable(R.drawable.oval_selected);
						SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(getActivity());
						preferences.edit().putBoolean(PackageKeys[loc - 1], !preferences.getBoolean(PackageKeys[loc - 1], false)).commit();
						cost[loc].setText("ON");
						cost[loc].setCompoundDrawablesWithIntrinsicBounds(null, drawable_selected, null, null);
					} else {
		...
		mHelper.startSetup(new IabHelper.OnIabSetupFinishedListener() {
			public void onIabSetupFinished(IabResult result) {
				if (!result.isSuccess()) {
					...
					additionalSkuList.add(SKU[6]);
					additionalSkuList.add(SKU[7]);
					additionalSkuList.add(SKU[8]);
		...
		mQueryFinishedListener = new IabHelper.QueryInventoryFinishedListener() {
			public void onQueryInventoryFinished(IabResult result, Inventory inventory) {
				if (result.isFailure()) {
					...
					// update UI
					Toast.makeText(getActivity(), "" + inventory.getSkuDetails("engineer"), Toast.LENGTH_SHORT).show();
					...
					Drawable drawable_selected = getResources().getDrawable(R.drawable.oval_selected);
					Drawable drawable_unselected = getResources().getDrawable(R.drawable.oval_unselected);
					int[] subs = { 2, 3, 6, 4, 5, 1 };
					int[] SKUsubs = { 3, 4, 5, 6, 7, 8 };
			...
			public void onConsumeFinished(Purchase purchase, IabResult result) {
				if (result.isSuccess()) {
					if (purchase.getSku().equals(SKU[0])) {
						updateMoney(Cost[0]);
						Money.increaseMoney(EggHelper.unlockEgg(getActivity(), moneyText, null, EggKeys[10], EggMaxValues[10]));
					} else if (purchase.getSku().equals(SKU[1])) {
						updateMoney(Cost[1]);
						Money.increaseMoney(EggHelper.unlockEgg(getActivity(), moneyText, null, EggKeys[11], EggMaxValues[11]));
					} else if (purchase.getSku().equals(SKU[2])) {
						updateMoney(Cost[2]);
						Money.increaseMoney(EggHelper.unlockEgg(getActivity(), moneyText, null, EggKeys[12], EggMaxValues[12]));
					}
				} else {
					// handle error
				}
		...
		TextView customPacks = (TextView) view.findViewById(R.id.unlock_custom);
		...
		TextView customPacksCost = (TextView) view.findViewById(R.id.custom_cost);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// "";
	...
	public void onResume() {
		super.onResume();
		setCost();
		Money.increaseMoney(EggHelper.unlockEgg(getActivity(), moneyText, null, EggKeys[5], EggMaxValues[5]));
	}
	...
	private void updateMoney(int amount) {
		...
		MoneyHelper.setMoney(getActivity(), moneyText, null, Money.getMoney(), Money.getMoneyPaid(), 0);
	}
	...
	private void buyProduct(String title, final int product, final int amount, final Intent i) {
		...
		if (product == 2 || product == 3 || product == 6 || product == 4 || product == 5 || product == 1) {
			if (!iabFinishedSetup) {
				alertIabNotSetup();
			} else {
				...
				mHelper.launchSubscriptionPurchaseFlow(getActivity(), SKU[SKUsub], Cost[SKUsub] + 1, mPurchaseFinishedListener,
						PURCHASE_KEY);
			}
		} else {
			AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
	...
	private void setCost() {
		...
		if (sharedPrefsMoney.getBoolean("unlock_all", false)) {
			...
			cost[0].setText("ON");
			// buy_layout[0].setEnabled(false);
			for (int i = 0; i < unlockPackageKeys.length; i++) {
				buy[i].setEnabled(false);
			}
		} else {
			cost[0].setText(String.valueOf(unlockCost[0]));
			cost[0].setCompoundDrawablesWithIntrinsicBounds(null, drawable_unselected, null, null);
		}
		...
		for (int i = 1; i < buy.length; i++) {
			if (sharedPrefsMoney.getBoolean(unlockPackageKeys[i], false) || sharedPrefsMoney.getBoolean(unlockSubPackageKeys[i], false)) {
				...
				boolean isOn = i != 0 && PreferenceManager.getDefaultSharedPreferences(getActivity()).getBoolean(PackageKeys[i - 1], true);
				cost[i].setText(isOn ? "ON" : "OFF");
				cost[i].setCompoundDrawablesWithIntrinsicBounds(null, isOn ? drawable_selected : drawable_unselected, null, null);
				buy[i].setEnabled(false);
				// buy_layout[i].setEnabled(false);
			} else if (i != 2 && i != 3 && i != 6 && i != 4 && i != 5 && i != 1) {
				cost[i].setText(String.valueOf(unlockCost[i]));
				cost[i].setCompoundDrawablesWithIntrinsicBounds(null, drawable_unselected, null, null);
			}
		}
		...
		// cost[a].setCompoundDrawables(null, price, null, null);
		// }
	}
